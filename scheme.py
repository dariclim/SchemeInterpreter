...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        procname = target.first
        procbody = LambdaProcedure(target.rest, expressions.rest, env)
        env.define(procname,procbody)
        return procname
        # END PROBLEM 9
    else:
...
def do_and_form(expressions, env):
    ...
    while expressions.rest != nil:
        ...
        if evaluated == False and evaluated is not 0:
            return evaluated
...
def do_or_form(expressions, env):
    ...
    while expressions.rest != nil:
        evaluated = scheme_eval(expressions.first, env)
        if evaluated != False or evaluated is 0:
            return evaluated
        expressions = expressions.rest
    return scheme_eval(expressions.first, env, True)
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    "*** YOUR CODE HERE ***"
    local_formals, local_vals = nil, nil
    while bindings is not nil:
        curr_bind = bindings.first
        check_form(curr_bind,2,2)
        local_formals = Pair(curr_bind.first, local_formals)
        local_vals = Pair(scheme_eval(curr_bind.rest.first, env), local_vals)
        bindings = bindings.rest
    check_formals(local_formals)
    local_frame = env.make_child_frame(local_formals, local_vals)
    return local_frame
    # END PROBLEM 14
...
def do_define_macro(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(macro_name, macro_body)
        return macro_name
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))
